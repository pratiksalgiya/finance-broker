...
> {
  ...
  handleBlur = () => {
    ...
    if (!/^[A-Za-z0-9 ]+$/.test(this.state.name)) {
      this.setState({
        errorMessage: "Name shouldn't contain any special characters",
        showError: true,
      });
    } else {
  ...
  handleMobileBlur = () => {
    if (this.state.mobileNumber.length > 10) {
      this.setState({
        errorMessageMobile: "Mobile Number can't contain more than 10 digits",
      ...
      <div>
        ...
        <div className='row'>
          ...
          {this.state.showError && (
            <span className='col-sm-4 padding-between error_msg'>
        ...
        <div className='row'>
          ...
          <div className='col-sm-2 padding-between'>
            <input
              ...
              onChange={(event) =>
                this.handleChange({ mobileNumber: event.target.value })
              }
            />
          </div>
        ...
        <div className='row form-buttons'>
          <div className='col-sm-1'>
            <button
              disabled={this.state.showError || this.state.showMobileError}
